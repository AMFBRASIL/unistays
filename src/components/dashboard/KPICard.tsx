    ...
    >
      ...
      <div className="relative">
        <div className="flex items-start justify-between mb-2 sm:mb-4">
          <div
            className={cn("p-1.5 sm:p-2.5 rounded-lg sm:rounded-xl bg-primary/10", iconColor.replace("text-", "bg-").replace(/\/\d+/, "/10"))}
          >
        ...
        <div>
          ...
          {changeLabel && <p className="text-[10px] sm:text-xs text-muted-foreground mt-1 sm:mt-2 truncate">{changeLabel}</p>}
        </div>
      </div>
    </div>
  );
